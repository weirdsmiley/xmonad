...
data Mode = Work | ShortBreak | LongBreak deriving (Eq, Show)
  ...
  let loop timeLeft pausedNow = do
      ...
      transition = do
        let nextMode = case mode of
              Work       -> if (count + 1) `mod` 4 == 0 then LongBreak else ShortBreak
              _          -> Work
            nextDuration = case nextMode of
              Work       -> w
              ShortBreak -> s
              LongBreak  -> l
            nextCount = if mode == Work then count + 1 else count
